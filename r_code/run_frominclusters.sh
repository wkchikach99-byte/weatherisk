...
do
	for j in "${alpha_list[@]}"
	do
		jid6=$(sbatch -J 6_inl${i}${j} --parsable  -o ${out}/6_inclusters_locest_${i}_${j}_%a.out c6_inclusters_locest.sh ${dirname} ${i} ${j})
		slurmids+=($jid6)
		...
		slurmids+=($jid7)
		jid8=$(sbatch -J 8_ins${i}${j} --parsable  -o ${out}/8_inclusters_saunders_${i}_${j}_%a.out c8_inclusters_saunders.sh ${dirname} ${i} ${j})
		slurmids+=($jid8)
