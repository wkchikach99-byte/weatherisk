...
rm -f $path/logs/*;
mkdir -p $path/res;
rm $path/res/* 2>/dev/null;
mkdir -p $path/res/tmp;
rm -f $path/res/tmp/*;

# run simulation
jid1=$(sbatch -J 1_simulate --parsable -o ${out}/1_simulation.out c1_simulation.sh ${dirname}) 
# local estimates
slurmids=()
for i in "${df_list[@]}"
do
	for j in "${alpha_list[@]}"
	do
		jid2=$(sbatch -J 2_lec${i}${j} --parsable --dependency=afterok:${jid1} -o ${out}/2_locest_calc_${i}_${j}_%a.out c2_locest_calc.sh ${dirname} ${i} ${j})
