...
do
	for j in "${alpha_list[@]}"
	do
		jid2=$(sbatch -J 2_lec${i}${j} --parsable -o ${out}/2_locest_calc_${i}_${j}_%a.out c2_locest_calc_override.sh ${dirname} ${i} ${j} ${locest})
		jid3=$(sbatch -J 3_lem${i}${j} --parsable --dependency=afterok:${jid2} -o ${out}/3_locest_merge_${i}_${j}.out c3_locest_merge_override.sh ${dirname} ${i} ${j} ${smoothing})
