...
Achieving this level of numerical equivalence required five targeted
adjustments to match subtle conventions in the R~code:
\begin{enumerate}
\item \emph{Optimiser scaling.}
  R's built-in optimiser internally rescales each parameter before
  searching.  The Python implementation applies the same rescaling
  so that both optimisers explore the same effective search space
  and converge to the same solution.
\item \emph{Boundary retries.}
  If an estimated parameter lands very close to the edge of its
  allowed range (within 1\% of the bound), the result may be
  unreliable.  In that case the optimisation is restarted from a
  new random initial point.  Up to five such retries are attempted,
  matching the strategy used in the R~code.
\item \emph{Array ordering.}
  R stores matrices column-by-column (column-major order), whereas
  Python's default is row-by-row.  When a matrix is converted to a
  flat list of numbers, the two languages produce different
  orderings.  The Python code explicitly uses column-major flattening
  so that every intermediate array matches the R output exactly.
\item \emph{Angle wrapping.}
  The rotation angle~$\gamma$ that describes the orientation of
  the spatial dependence ellipse is periodic: $+90°$ and $-90°$
  represent the same direction.  Whenever the optimiser pushes
  $\gamma$ beyond this range, the Python code reflects it back,
  reproducing the same wrapping convention used in R.
\item \emph{Likelihood averaging.}
  The log-likelihood used for estimation is a sum over all pairs
  of neighbouring grid cells.  The R~code divides this sum by the
  number of pairs before optimising; the Python code applies the
  same division so that the magnitude of the objective function---and
  hence the optimiser's convergence behaviour---is identical.
\end{enumerate}
Together, these adjustments ensure that published results based on
