...
Heatwaves and heavy rainfall events rarely affect a single location
in isolation.  When one city records an extreme temperature, nearby
cities usually experience the same event.  Understanding \emph{where}
extreme weather tends to strike together is important for disaster
planning, insurance, and climate adaptation.  If this spatial
co-occurrence is ignored, the total impact of a large-scale event
can be severely underestimated~[1].

The statistical theory of \emph{max-stable processes} provides a
natural way to describe how extremes at different locations depend on
each other~[2,3].  In practice, one fits local parameters that
characterise how strongly (and in which direction) extreme events
at each grid point are linked to their neighbours.  Once these
parameters are estimated, grid points with similar dependence
structure can be grouped into \emph{clusters}---regions where
extreme events behave in a coherent way.
...
intermediate results to machine precision---and adds several
practical features:
\begin{itemize}
...
No existing Python package offers this combination of max-stable
simulation, local composite-likelihood estimation, and
ellipse-based spatial clustering.  Packages such as
\texttt{pyextremes}~[5] and \texttt{scikit-extremes} focus on
univariate extreme-value analysis; the R package
...
\item \textbf{Data preparation.}
  ...
  grid cell.  The fitted values are transformed to a common unit
  (Fr\'{e}chet margins) so that spatial dependence can be compared
  across locations.
...
\item \textbf{Local estimation.}
  ...
  pairwise composite likelihood over neighbouring pairs~[3].
  Estimates are then spatially smoothed.
...
\item \textbf{EDC} (Extremal Dependence Clustering): based on
...
EDC clusters, panel~(b) the LEC clusters, and panel~(c) the true
parameter field.  Both methods successfully recover the stripe
boundaries, with LEC producing sharper transitions that closely
match the ground truth.  This confirms that the Python
implementation correctly reproduces the clustering methodology
of Contzen et al.~[4].

\begin{figure*}[ht]
\centering
\includegraphics[width=0.95\textwidth]{figures/stripes_clusters_b.pdf}
\caption{Synthetic validation on the ``stripes'' preset.
  (a)~EDC clusters coloured by estimated $b$;
  (b)~LEC clusters coloured by estimated $b$;
  (c)~true parameter field.
  Both methods recover the stripe structure; LEC produces
  boundaries closer to the true transitions.}
\label{fig:stripes}
\end{figure*}
...
The method identifies $k = 26$ clusters based on ellipse-overlap
dissimilarity with a 30\%-quantile threshold.  The spatial
pattern is physically interpretable: the Mediterranean basin,
Atlantic-influenced Western Europe, Scandinavia, and continental
interiors are assigned to different clusters, consistent with
known climatic boundaries.  The full pipeline---data loading,
GEV fitting, local estimation, smoothing, clustering, and map
generation---runs in approximately 100~seconds on an Apple~M2
laptop with 16\,GB RAM.

\begin{figure}[ht]
\centering
\includegraphics[width=0.95\columnwidth]{figures/lec_clusters_map.pdf}
\caption{LEC spatial dependence clusters for precipitation
  extremes (2000--2019, $k = 26$).  Each colour identifies a
  region where extreme precipitation events share a similar
...
produced from the same data.  The EDC method, which uses
madogram-based extremal coefficients rather than locally estimated
ellipse parameters, identifies $k = 18$ clusters.  Because the
madogram captures a different aspect of spatial dependence (the
average tendency of two locations to jointly exceed high
thresholds), the resulting clusters are coarser and more fragmented
than the LEC clusters.  Together, the two methods provide
complementary views of the dependence structure: LEC captures the
shape and orientation of spatial dependence, while EDC captures its
overall strength.

\begin{figure}[ht]
\centering
\includegraphics[width=0.95\columnwidth]{figures/edc_clusters_map.pdf}
\caption{EDC spatial dependence clusters for precipitation
  extremes (2000--2019, $k = 18$).  Compared with the LEC
  clusters in Figure~\ref{fig:lec-map}, the EDC clusters are
  coarser and more fragmented, reflecting a different measure
...
cross-validation and R-parity tests.  These tests execute the
original R functions (via exported reference data), compare every
intermediate result---from individual covariance evaluations to
final cluster assignments---and assert agreement to machine
precision.  Five specific algorithmic fixes (parameter rescaling,
boundary retries, column-major indexing, gamma-wrapping, and
average log-likelihood computation) were implemented to achieve
exact numerical equivalence with R.  This level of verification
gives confidence that published results based on the R code can be
faithfully reproduced in Python.
...
\paragraph{Education.}
The step-by-step pipeline design, together with the comprehensive
test suite, makes the package suitable for graduate-level teaching
of spatial extreme-value methods.

\paragraph{Scalability.}
The coarse-grid proxy strategy and embarrassingly parallel local
estimation allow the method to be applied to global grids with
over 250\,000 cells on standard university HPC infrastructure.
...
The authors thank [to be completed] for computational resources and
helpful discussions.
