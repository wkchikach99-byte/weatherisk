...
\rev{\subsection{HPC deployment}
\label{sec:hpc}

\revb{Revisiting} the full Figure~9 experiment of Contzen et al.~[4]
requires processing AWI-ESM-1-1-LR historical precipitation on the
T63 grid ($192 \times 96 = 18{,}432$ cells, 156~years: 1850--2005).
At this scale, the local MLE step alone must solve 18{,}432
optimisation problems (one per grid cell, five restarts each), and
the full pairwise dissimilarity matrix contains $\sim 3.4 \times
10^8$ entries ($\sim 1.7 \times 10^8$ unique pairs). \revo{This scale is too large for a standard laptop, which is why the
experiment was run on the AWI \emph{albedo} HPC cluster.}

We executed the Figure~9 \revb{case-study workflow} on the AWI \emph{albedo}
high-performance computing facility with the following SLURM
configuration:
...
\midrule
Partition & \texttt{smp} (shared memory) \\
Nodes / tasks & 1 / 1 \\
CPUs per task & 16 \\
Memory & 220\,GiB \\
Wall-time limit & 48\,h \\
Software & Python 3.11.7, R 4.2.2 \\
Thread control & \texttt{OMP/MKL/OPENBLAS\_NUM\_THREADS=1} \\
\bottomrule
\end{tabular}
\caption{SLURM configuration used for the full Figure~9 \revb{case-study run}
  on the AWI \emph{albedo} high-performance computing facility.}
\label{tab:hpc-config}
\end{table}

\FloatBarrier

\revg{The data volume associated with modern Earth system
simulations is itself substantial. For the AWI-ESM-1-1-LR Figure~9
experiment considered here, a single monthly variable on the native
T63 grid corresponds to $192 \times 96 \times 156 \times 12 =
34{,}504{,}704$ data values, that is, already of order $10^7$ for one
field. For multi-variable workflows, this readily increases to order
$10^8$--$10^9$ values across the model output archive. At higher
spatial resolution, the scaling becomes much steeper: moving from the
T63 grid to a global $0.25^{\circ}$ grid increases the number of grid
cells by roughly a factor of 56, and replacing monthly output by daily
output introduces an additional factor of about 30. Combined, these
changes yield an increase of more than three orders of magnitude in
raw data volume relative to the present CMIP6-class example.}

\revg{This scaling is especially important because the pairwise
dependence stage grows quadratically in the number of grid cells. A
transition from $\sim 1.8 \times 10^4$ cells to $\sim 10^6$ cells
therefore increases the leading-order pairwise cost by roughly
$10^4$. This is precisely why future GPU acceleration, hybrid CPU--GPU
workflows, and approximate sparse or graph-based strategies are not
only attractive, but likely necessary for next-generation
kilometre-scale Earth system applications.}

\revo{Within the broader scientific Python ecosystem, \texttt{xarray}~[13]
provides labelled multi-dimensional arrays well suited to NetCDF-based
climate workflows. The \texttt{dask}~[12] library offers blocked
parallel array execution for larger-than-memory datasets, and
community platforms such as Pangeo~[14] integrate these and related
tools into scalable geoscientific-analysis environments spanning
workstations, HPC systems, and cloud deployments. In the current
version of \texttt{weatherisk}, \texttt{xarray} is used for
climate-data I/O and lazy loading; \texttt{dask} and Pangeo are not
current runtime dependencies of \texttt{weatherisk} but represent an
established ecosystem context for scaling geoscientific workflows of
this type.}

\noindent Setting all threading libraries to single-threaded
operation prevents over-subscription\revo{ (too many threads competing for
the same CPU cores)} when the 16~worker processes are active.
% TODO (VPN required): replace the sentence below with the exact elapsed
% time from job 42260990.  Run on albedo:
%   grep -E "Elapsed|completed" ~/weatherisk/output/cmip6_fig9/logs/fig9_42260990.out
% Then write: "The validated production run (job~42260990) completed in
% Xs (Y.Z\,h); local MLE (Step~3) accounted for Zs of that total,
% with Steps~5 and~6 dominating at this grid scale."
% DO NOT use numbers from job 42197121 (wrong cluster counts).
The validated production run (job~42260990, 16~workers) completed
well within the 48~h reservation; exact step-level timings will be
inserted here once the SLURM log has been retrieved~[to be confirmed].
Peak memory use was below 2\,GiB, well within the 220\,GiB
allocation.

The SLURM job script, environment setup, and the exact command-line
invocation are provided in the repository under \texttt{hpc/}
(files \texttt{run\_fig9\_clean.slurm}, \texttt{setup\_albedo.sh}).
\revo{Each run is tagged with a unique SLURM job identifier, so all output
files can be traced back to a specific job submission.} Checkpoint files
allow the pipeline to resume from the last completed stage after
interruptions.}
