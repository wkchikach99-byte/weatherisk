...
kilometres, illustrating that extreme precipitation events are
...
cross-validation and R-parity tests.  These tests compare key intermediate results---from individual
covariance evaluations to final cluster assignments---against
reference values exported from the original R code, asserting
agreement to within $10^{-10}$.  Five specific algorithmic fixes (parameter rescaling,
...
test suite and documented code examples (Appendix~\ref{app:code}),
makes the package a potential resource for graduate-level teaching
of spatial extreme-value methods.

\paragraph{Scalability.}
The coarse-grid proxy strategy and embarrassingly parallel local
estimation are designed to allow the method to scale to global
grids with hundreds of thousands of cells on standard HPC
hardware; benchmarking at that scale is planned for a future
release.
